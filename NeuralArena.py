...
def upload_dataset():
    ...
    # Ensure both upload and data directories exist
...
def train_model():
    ...
    model_cost = memory_costs.get(model_name, 10)
    has_memory_crystal = 'memory_crystal' in player_data.get('purchasedItems', [])
    memory_capacity = 1000 if has_memory_crystal else 100
    
    if model_cost > memory_capacity:
        return jsonify({
            "status": "error",
            "message": "Memory capacity exceeded! You need a Memory Crystal to train this model."
        }), 400
...
def classify_image():
    ...
    try:
        ...
        img_path = os.path.join(app.config['UPLOAD_FOLDER'], 'temp_classify.jpg')
        file.save(img_path)
        ...
        model = get_model(model_name, num_classes=1000)  # ImageNet classes
        model.eval()
        ...
        predictions = [
            {"label": f"Class {idx.item()}", "confidence": prob.item() * 100}
...
@app.route('/random-cifar', methods=['GET'])
def random_cifar():
    try:
        # Load CIFAR-10 if not already loaded
        dataset = datasets.CIFAR10(root=DATA_FOLDER, train=False, download=True)
        ...
        # Simulate predictions (replace with real model predictions if needed)
        predictions = [
            {"label": f"Class {label}", "confidence": random.uniform(60, 90)}
        ]
        for _ in range(4):
            fake_label = random.randint(0, 9)
            if fake_label != label:
                predictions.append({
                    "label": f"Class {fake_label}",
...
def process_image():
    ...
    try:
        ...
        time.sleep(processing_times.get(model_name, 0.1))
...
def test_accuracy():
    ...
    # Validate model name
    if model_name not in ['SimpleCNN', 'vgg16', 'resnet50', 'efficientnet', 'inception']:
        print(f"Error: Invalid model name: {model_name}")
        return jsonify({"status": "error", "message": "Invalid model name"}), 400
        
    try:
        # Base accuracy for each model type
        base_accuracies = {
            'SimpleCNN': 0.75,
            'vgg16': 0.92,
            'resnet50': 0.94,
        ...
        # Add slight randomness but maintain model characteristics
        random_factor = random.uniform(0.95, 1.05)  # ±5% variation
        adjusted_acc = min(1.0, base_acc * random_factor)
        
        # Determine if this attempt was successful
        is_correct = random.random() < adjusted_acc
        
        # Log progress
        print(f"\nAccuracy test iteration {iteration}/100:")
        print(f"  Model: {model_name}")
        print(f"  Base accuracy: {base_acc:.3f}")
        print(f"  Adjusted accuracy: {adjusted_acc:.3f}")
        ...
        return jsonify({
            "status": "error",
            "message": f"Error during accuracy test: {str(e)}"
        }), 500
