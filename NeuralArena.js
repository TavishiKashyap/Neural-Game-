document.addEventListener('DOMContentLoaded', () => {
    const game = {
        ...
        // Model Memory Costs (percentage of total capacity)
        memoryCosts: {
            'SimpleCNN': 10,     // Basic model (10%)
            'vgg16': 120,        // Heavy model (120% - needs crystal)
            'resnet50': 110,     // Heavy model (110% - needs crystal)
            'efficientnet': 40,  // Medium model (40%)
            'inception': 100,    // Large model (100% - fits exactly)
        },
        ...
        initTrainButtons() {
            document.querySelectorAll('.train-button').forEach(button => {
                ...
                newButton.addEventListener('click', () => {
                    ...
                    const modelCost = this.memoryCosts[this.equippedModel] || 0;
                    const hasCrystal = this.purchasedItems.includes('memory_crystal');
                    const maxCapacity = hasCrystal ? 1000 : 100;
                    
                    if (modelCost > maxCapacity) {
                        this.showNotification('Cannot train - Memory capacity exceeded! Equip a Memory Crystal for larger models.', 'error');
                        return;
                    }
                ...
                newButton.addEventListener('click', () => {
                    ...
                    if (isBuy) {
                        ...
                        if (itemType === 'project') {
                            ...
                            const modelCost = this.memoryCosts[itemId] || 0;
                            const hasCrystal = this.purchasedItems.includes('memory_crystal');
                            const maxCapacity = hasCrystal ? 1000 : 100;
                            
                            if (modelCost > maxCapacity) {
                                this.showNotification(`Cannot equip ${itemId} - Memory Crystal required!`, 'error');
                                return;
                            }
        ...
        showScreen(screenId) {
            ...
            if (activeScreen) {
                ...
                // Reset game states when switching screens
                if (screenId === 'data-garden-screen') {
                    // Reset mini-game states
                    this.gameState.accuracyTest.isRunning = false;
                    this.gameState.accuracyTest.progress = 0;
                    this.gameState.accuracyTest.score = 0;
                    this.gameState.speedTest.isRunning = false;
                    this.gameState.speedTest.images = 0;
                    this.gameState.speedTest.timeLeft = 10.0;
                    this.gameState.classifier.isProcessing = false;
                    
                    // Reset UI elements
                    const progressSpan = document.getElementById('accuracy-progress');
                    const scoreSpan = document.getElementById('accuracy-score');
                    const progressBar = document.getElementById('accuracy-bar');
                    const speedTimer = document.getElementById('speed-timer');
                    const speedImages = document.getElementById('speed-images');
                    
                    if (progressSpan) progressSpan.textContent = '0';
                    if (scoreSpan) scoreSpan.textContent = '0.0';
                    if (progressBar) progressBar.style.width = '0%';
                    if (speedTimer) speedTimer.textContent = '10.0';
                    if (speedImages) speedImages.textContent = '0';
                    
                    // Re-enable buttons
                    const accuracyButton = document.getElementById('start-accuracy-test');
                    const speedButton = document.getElementById('start-speed-test');
                    if (accuracyButton) accuracyButton.disabled = false;
        ...
        updateMemoryBar() {
            ...
            // Get crystal status and calculate max capacity
            const crystalActive = this.purchasedItems.includes('memory_crystal');
            const maxCapacity = crystalActive ? 1000 : 100;
            
            // Update crystal slot display
            if (crystalActive) {
            ...
            // Calculate memory usage for equipped model
            const memoryUsed = this.equippedModel ? (this.memoryCosts[this.equippedModel] || 0) : 0;

            if (!fillBar || !fillText || !crystalSlot) return; // Exit if UI not loaded

            // Calculate fill percentage (cap at 100% for display)
            const fillPercent = Math.min((memoryUsed / maxCapacity) * 100, 100);
            
            // Update the visual bar
            fillBar.style.width = `${fillPercent}%`;
            
            // Update the text display
            fillText.textContent = `${memoryUsed}% Used / ${maxCapacity}% Capacity`;
        },
        ...
        initMiniGames() {
            ...
            const uploadButton = document.getElementById('upload-test-image');
            const imageInput = document.getElementById('test-image-input');
            const randomButton = document.getElementById('random-cifar-test');
            
            if (uploadButton) {
                uploadButton.addEventListener('click', () => {
                    if (!this.equippedModel) {
                        this.showNotification('Please equip a model first!', 'error');
                        return;
                    }
                    imageInput.click();
                });
            ...
            if (imageInput) {
                imageInput.addEventListener('change', (e) => {
                    ...
                    if (file) {
                        ...
                        reader.onload = (e) => {
                            const img = document.getElementById('classifier-image');
            ...
            // Accuracy Challenge
            const accuracyButton = document.getElementById('start-accuracy-test');
            if (accuracyButton && !accuracyButton.dataset.listenerAdded) {
                accuracyButton.addEventListener('click', async () => {
                    console.log('Accuracy test button clicked');
                    if (!this.equippedModel) {
                        this.showNotification('Please equip a model first!', 'error');
                        return;
                    }
                    try {
                        console.log('Starting accuracy test with model:', this.equippedModel);
                        accuracyButton.disabled = true;
                        await this.startAccuracyChallenge();
                        accuracyButton.disabled = false;
                    } catch (error) {
                        console.error('Error in accuracy test:', error);
                        this.showNotification('Error running accuracy test: ' + error.message, 'error');
                        accuracyButton.disabled = false;
                    }
                });
                accuracyButton.dataset.listenerAdded = 'true';
            } else if (!accuracyButton) {
                console.error('Accuracy test button not found');
            }
        },
        ...
        async classifyImage(file) {
            const formData = new FormData();
            formData.append('file', file);
            formData.append('modelName', this.equippedModel);

            try {
                const response = await fetch(`${this.backendUrl}/classify-image`, {
                    method: 'POST',
                    body: formData
                });
                ...
                if (result.status === 'success') {
                    this.showPredictions(result.predictions);
                } else {
                    this.showNotification('Classification failed!', 'error');
                }
            } catch (error) {
        ...
        async getRandomCIFARImage() {
            ...
            try {
                // First get a random CIFAR image
                const response = await fetch(`${this.backendUrl}/random-cifar`);
                const result = await response.json();
                
                if (result.status === 'success') {
                    // Display the image
                    const img = document.getElementById('classifier-image');
                    img.style.backgroundImage = `url(data:image/jpeg;base64,${result.image})`;
                    
                    // Now test the model's prediction accuracy on this image
                    const testResponse = await fetch(`${this.backendUrl}/test-accuracy`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            modelName: this.equippedModel,
                    ...
                    if (testResult.status === 'success') {
                        ...
                        const predictions = [
                            { label: `${testResult.correct ? 'Correct' : 'Incorrect'} Prediction`, 
                              confidence: testResult.correct ? 100 : 0 }
                        ];
                        this.showPredictions(predictions);
                    }
                }
            } catch (error) {
                console.error('Error in random CIFAR test:', error);
                this.showNotification('Error testing model', 'error');
            }
        },

        showPredictions(predictions) {
            const resultsDiv = document.getElementById('classifier-results');
            const barsDiv = document.getElementById('prediction-bars');
            
            if (!resultsDiv || !barsDiv) return;
            
            resultsDiv.style.display = 'block';
            barsDiv.innerHTML = predictions
                .map(p => `
                    <div class="prediction-bar" style="--confidence: ${p.confidence}%">
                        <span class="label">${p.label}</span>
                        <span class="confidence">${p.confidence.toFixed(1)}%</span>
                    </div>
                `).join('');
        },

        async startSpeedChallenge() {
            const statsDiv = document.getElementById('speed-stats');
            const timerSpan = document.getElementById('speed-timer');
            const imagesSpan = document.getElementById('speed-images');
            const startButton = document.getElementById('start-speed-test');
            
            if (!statsDiv || !timerSpan || !imagesSpan || !startButton) return;
            ...
            const timer = setInterval(() => {
                ...
                timerSpan.textContent = timeLeft.toFixed(1);
                
                if (timeLeft <= 0) {
                    clearInterval(timer);
                    startButton.disabled = false;
                    this.showNotification(`Speed test complete! Processed ${images} images`, 'success');
                    // Save high score?
                }
            }, 100);
            ...
            try {
                while (timeLeft > 0) {
                    const response = await fetch(`${this.backendUrl}/process-image`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ modelName: this.equippedModel })
                    });
                    
                    if (timeLeft > 0) {  // Only count if we're still within time
                        images++;
                        imagesSpan.textContent = images;
                    }
                }
            } catch (error) {
                clearInterval(timer);
                startButton.disabled = false;
                this.showNotification('Error during speed test', 'error');
            }
        },

        async startAccuracyChallenge() {
            console.log('Starting accuracy challenge...');
            
            // Prevent multiple runs
            if (this.gameState.accuracyTest.isRunning) {
                console.log('Test already running');
                this.showNotification('Test is already running', 'error');
                return;
            }
            
            // First check if model is equipped
            if (!this.equippedModel) {
                this.showNotification('Please equip a model first!', 'error');
                return;
            }
            ...
            console.log(`Starting accuracy test with model: ${this.equippedModel}`);
            this.showNotification(`Testing ${this.equippedModel}'s accuracy on random images...`, 'unlock');
            this.gameState.accuracyTest.isRunning = true;

            // Get UI elements
            const progressSpan = document.getElementById('accuracy-progress');
            const scoreSpan = document.getElementById('accuracy-score');
            const progressBar = document.getElementById('accuracy-bar');
            const startButton = document.getElementById('start-accuracy-test');
            ...
            if (!progressSpan || !scoreSpan || !progressBar || !startButton) {
                console.error('Missing UI elements for accuracy challenge');
                this.showNotification('Game initialization error', 'error');
                return;
            }
            ...
            try {
                // Reset progress display and state
                ...
                console.log('Starting test iterations...');
                // Run 100 test iterations
                for (let i = 1; i <= 100; i++) {
                    console.log(`Making request ${i}/100 for model ${this.equippedModel}`);
                    try {
                        const response = await fetch(`${this.backendUrl}/test-accuracy`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ 
                                modelName: this.equippedModel,
                        ...
                        console.log(`Response status for iteration ${i}: ${response.status}`);
                        ...
                        if (result.status === 'success') {
                            if (result.correct) {
                                correct++;
                                console.log(`Correct prediction! Total correct: ${correct}/${i}`);
                            }
                            
                            // Update UI with current progress
                            progressSpan.textContent = i;
                            const currentAccuracy = ((correct / i) * 100).toFixed(1);
                            scoreSpan.textContent = currentAccuracy;
                            progressBar.style.width = `${i}%`;
                            
                            // Show intermediate updates every 10 iterations
                            if (i % 10 === 0) {
                                this.showNotification(`Testing progress: ${i}% complete. Current accuracy: ${currentAccuracy}%`, 'unlock');
                            }
                            
                            // Add small delay to prevent UI freezing and allow updates to render
                        ...
                        // Update game state
                        this.gameState.accuracyTest.progress = i;
                        this.gameState.accuracyTest.score = (correct / i) * 100;
                        
                    } catch (iterationError) {
                        console.error(`Error in iteration ${i}:`, iterationError);
                        this.showNotification(`Error in test iteration ${i}: ${iterationError.message}`, 'error');
                ...
                const finalScore = (correct / 100) * 100;
                this.showNotification(`Accuracy test complete! Score: ${finalScore.toFixed(1)}%`, 'success');
                
                // Award credits based on score
                const creditsEarned = Math.floor(finalScore);
                if (creditsEarned > 0) {
                    this.computeCredits += creditsEarned;
                    this.updatePlayerStats();
                    this.showNotification(`Earned ${creditsEarned} credits for good performance!`, 'success');
                }
                
            } catch (error) {
                console.error('Accuracy challenge error:', error);
                this.showNotification(`Error during accuracy test: ${error.message}`, 'error');
